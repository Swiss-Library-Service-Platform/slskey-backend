...
it('succeeds to authenticate user', function ($almaUser) {
    ...
    session(['alma_institution' => $almaIz]);
    test()->mock(AuthCloudApp::class, function ($mock) use ($almaUsername, $almaIz) {
        $mock->shouldAllowMockingProtectedMethods();
        $mock->makePartial();
        $mock->shouldReceive('extractTokenFromRequest')->andReturn('token');
        $mock->shouldReceive('decodeJWTToken')->andReturn((object) ['sub' => $almaUsername, 'inst_code' => $almaIz]);
        $mock->shouldReceive('checkUserHasAlmaPermissions')->andReturn(['success' => true]);
    });
    ...
    assertAdminUserExisting($almaUsername);
})->with([
